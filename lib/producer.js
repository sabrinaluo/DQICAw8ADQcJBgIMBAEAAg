...
class Producer {
	...
	/**
	 * put job into tube
	 * @param {Seed} payload - job payload
		...
		return new Promise((resolve, reject) => {
			co(function* () {
				...
				try {
					...
					let job_id = yield client.putAsync(PRIORITY, delay, TIME_TO_RUN, JSON.stringify(payload));
					client.end();
					resolve(job_id);
					log.info('put job success', {payload: payload, job_id: job_id});
				} catch (e) {
					reject(e);
